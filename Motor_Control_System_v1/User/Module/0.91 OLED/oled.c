...
/**
 * @brief 设置垂直滚动(向上或向下)
 * @param dir 方向: 0-向上滚动, 1-向下滚动
 *
 * SSD1306垂直滚动命令说明:
 * 0x29 - 垂直向右滚动 (Vertical and Right Horizontal Scroll)
 * 0x2A - 垂直向左滚动 (Vertical and Left Horizontal Scroll)
 *
 * 参数格式:
 * Byte1: 0x29或0x2A
 * Byte2: Dummy byte (0x00)
 * Byte3: 起始页地址 (0x00)
 * Byte4: 时间间隔 (0x00~0x07,越小越快)
 * Byte5: 结束页地址 (0x03, 因为32像素高度=4页)
 * Byte6: 垂直滚动偏移量 (0x01表示每次滚动1行)
 */
void OLED_Set_Vertical_Scroll(uint8_t dir)
{
    // 先停止滚动
    OLED_Stop_Scroll();

    if(dir == 0)  // 向上滚动 (使用垂直+右滚动)
    {
        OLED_Write_cmd(0x29);  // 垂直向右滚动命令
        OLED_Write_cmd(0x00);  // Dummy byte
        OLED_Write_cmd(0x00);  // 起始页地址 (第0页)
        OLED_Write_cmd(0x05);  // 滚动时间间隔 (0x05=中速)
        OLED_Write_cmd(0x03);  // 结束页地址 (第3页, 32像素=4页)
        OLED_Write_cmd(0x01);  // 垂直滚动偏移量 (每次滚动1行)
    }
    else  // 向下滚动 (使用垂直+左滚动,反向效果)
    {
        OLED_Write_cmd(0x2A);  // 垂直向左滚动命令
        OLED_Write_cmd(0x00);  // Dummy byte
        OLED_Write_cmd(0x00);  // 起始页地址 (第0页)
        OLED_Write_cmd(0x05);  // 滚动时间间隔 (0x05=中速)
        OLED_Write_cmd(0x03);  // 结束页地址 (第3页)
        OLED_Write_cmd(0x01);  // 垂直滚动偏移量 (每次滚动1行)
    }
